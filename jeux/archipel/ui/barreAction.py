...
class BarreAction:
    ...
    def dessine(self, manche: int, derniereManche: int) -> None:
        """Dessine la barre à l'écran

        Args:
            manche (int): Manche actuel de la partie.
            derniereManche (int): Manche final.
        """
        draw_rectangle(0, yf-self.hauteur, xf, yf, [80, 80, 80, 150])
        self.dessineInfoBat(manche, derniereManche)
    ...
    def dessineInfoBat(self, manche: int, derniereManche: int) -> None:
        ...
        if isinstance(bateau, Bateau):
            ...
            x += int(xf*0.025)
            draw_text_ex(police2, f"MANCHE {str(manche)}/{str(derniereManche)}", (x, y), yf*0.02, 0, WHITE)
