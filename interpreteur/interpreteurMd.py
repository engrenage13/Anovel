...
class InterpreteurMd:
    ...
    def erreur(self, mode: int) -> None:
        ...
        """
        if mode == 1:
            self.zone.ajouteContenu(['cad', BlocTexte("Le fichier n'existe pas ou n'est pas au bon endroit.", 
                                       police2, self.taillePolice, [self.zone.largeurContenu, ''])])
        elif mode == 2:
            self.zone.ajouteContenu(['cad', BlocTexte("Ce fichier n'est pas prit en compte", 
                                       police2, self.taillePolice, [self.zone.largeurContenu, ''])])